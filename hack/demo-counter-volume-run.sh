...
# Ensure the go install path is added to the PATH for the plugin execution
export GOPATH="${GOPATH:-$(go env GOPATH)}"
export GOBIN="${GOBIN:-$(go env GOBIN)}"
export PATH="${GOBIN}:${GOPATH}/bin:${PATH}"

log_step "Creating counter-volume actor (my-counter-volume-1)..."
kubectl ate create actor my-counter-volume-1 --template ate-demo-counter-volume/counter-volume

log_success "Counter-volume actor my-counter-volume-1 created"
echo ""
echo -e "${COLOR_YELLOW}========================================================================${COLOR_RESET}"
echo -e "To interact with the counter-volume actor, open a separate terminal and run:"
echo -e "  curl -X POST -H \"Host: my-counter-volume-1.actors.resources.substrate.ate.dev\" -i http://localhost:8000/"
echo -e "${COLOR_YELLOW}========================================================================${COLOR_RESET}"
echo ""
