...
// this probably requirs us to add a PROVISIONING actor state.
...
func (s *Service) createActorVolumes(ctx context.Context, actorID string, template *atev1alpha1.ActorTemplate) ([]*ateapipb.ExternalVolume, error) {
	...
	for _, vol := range template.Spec.Volumes {
		if vol.ExternalVolumeTemplate != nil {
			...
			storageVolumeID, err := getVolumePlugin().CreateVolume(ctx, uniqueVolName, vol.ExternalVolumeTemplate.Capacity, vol.ExternalVolumeTemplate.StorageClassName)
			if err != nil {
			...
			volumes = append(volumes, &ateapipb.ExternalVolume{
				...
				VolumeType:      vol.ExternalVolumeTemplate.StorageClassName,
