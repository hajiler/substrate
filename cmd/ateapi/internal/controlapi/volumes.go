...
func (s *Service) createActorVolumes(ctx context.Context, ref *ateapipb.ObjectRef, template *atev1alpha1.ActorTemplate) ([]*ateapipb.ExternalVolume, error) {
	...
	for _, vol := range template.Spec.Volumes {
		if vol.ExternalVolumeTemplate != nil {
			...
			plugin, err := s.GetPlugin(ctx, sc.Provisioner)
			if err != nil {
				return nil, status.Errorf(codes.FailedPrecondition, "failed to get volume plugin for driver %q (StorageClass %q): %v", sc.Provisioner, scName, err)
			}
			...
			if err != nil {
				// TODO: improve handling for potential leaked volumes.
