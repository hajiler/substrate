...
func Validate_ExternalVolume(
	...
	{ // field ateapipb.ExternalVolume.ReclaimPolicy
		fn := func(
			fldPath *field.Path,
			obj, oldObj *ateapipb.ReclaimPolicy,
			oldValueCorrelated bool) (errs field.ErrorList) {
			// don't revalidate unchanged data
			if oldValueCorrelated && op.Type == operation.Update {
				if obj == oldObj || (obj != nil && oldObj != nil && *obj == *oldObj) {
		...
		oldVal := safe.Field(oldObj,
			func(oldObj *ateapipb.ExternalVolume) *ateapipb.ReclaimPolicy {
				return &oldObj.ReclaimPolicy
			})
		errs = append(errs, fn(fldPath.Child("reclaim_policy"), &obj.ReclaimPolicy, oldVal, oldObj != nil)...)
	}
