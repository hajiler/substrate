...
func newTestExternalVolumeWorkflow(t *testing.T, reclaim corev1.PersistentVolumeReclaimPolicy) (*ExternalVolumeWorkflow, store.Interface, *recordingVolumePlugin) {
	t.Helper()
...
func TestCreateExternalVolume_Provisioned(t *testing.T) {
	...
	w, _, plugin := newTestExternalVolumeWorkflow(t, corev1.PersistentVolumeReclaimDelete)
	...
	if want := ateapipb.ReclaimPolicy_RECLAIM_POLICY_DELETE; got.GetReclaimPolicy() != want {
		t.Errorf("reclaim_policy = %v, want the class's %v", got.GetReclaimPolicy(), want)
	}
...
// TestCreateExternalVolume_Registered checks that a volume that already exists
// is recorded as-is: nothing is provisioned, and it is immediately usable.
func TestCreateExternalVolume_Registered(t *testing.T) {
	...
	// Collecting a disk Substrate did not make is not ours to do by default.
	if want := ateapipb.ReclaimPolicy_RECLAIM_POLICY_RETAIN; got.GetReclaimPolicy() != want {
		t.Errorf("reclaim_policy = %v, want %v", got.GetReclaimPolicy(), want)
	}
}
...
func TestCreateExternalVolume_Errors(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			w, _, _ := newTestExternalVolumeWorkflow(t, corev1.PersistentVolumeReclaimDelete)
...
// AlreadyExists rather than a second disk: retrying a create means deleting the
// volume first, which collects whatever the failed attempt stranded.
...
func TestDeleteExternalVolume(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "delete policy collects the disk", reclaim: corev1.PersistentVolumeReclaimDelete, wantDeleted: true},
		{name: "retain policy leaves the disk", reclaim: corev1.PersistentVolumeReclaimRetain},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			w, persistence, plugin := newTestExternalVolumeWorkflow(t, tc.reclaim)
			...
			if _, err := w.DeleteExternalVolume(ctx, volumeRef); err != nil {
				t.Fatalf("DeleteExternalVolume: %v", err)
			}

			var want []string
			if tc.wantDeleted {
				want = []string{created.GetVolumeId()}
			}
...
// TestDeleteExternalVolume_StillReferenced checks that a volume an actor holds
// cannot be pulled out from under it.
func TestDeleteExternalVolume_StillReferenced(t *testing.T) {
	ctx := context.Background()
	w, persistence, plugin := newTestExternalVolumeWorkflow(t, corev1.PersistentVolumeReclaimDelete)
	...
	_, err = w.DeleteExternalVolume(ctx, volumeRef)
...
// TestDeleteExternalVolume_Pending checks that a create that died is cleaned up
// by deleting the volume: with no handle recorded, the disk the create would
// have asked for is collected by the ID derived from the record's UID.
func TestDeleteExternalVolume_Pending(t *testing.T) {
	...
	if _, err := w.DeleteExternalVolume(ctx, resources.ExternalVolumeRefFromExternalVolume(pending)); err != nil {
		t.Fatalf("DeleteExternalVolume: %v", err)
	}
...
func TestDeleteExternalVolume_NotFound(t *testing.T) {
	...
	_, err := w.DeleteExternalVolume(ctx, resources.ExternalVolumeRef{Atespace: "team-a", Name: "nope"})
...
func TestDefaultReclaimPolicy(t *testing.T) {
	...
	recyclePolicy := corev1.PersistentVolumeReclaimRecycle
	...
	tests := []struct {
		...
		{name: "no class", want: ateapipb.ReclaimPolicy_RECLAIM_POLICY_RETAIN},
		{name: "class without a policy", sc: &storagev1.StorageClass{}, want: ateapipb.ReclaimPolicy_RECLAIM_POLICY_RETAIN},
		{name: "delete", sc: &storagev1.StorageClass{ReclaimPolicy: &deletePolicy}, want: ateapipb.ReclaimPolicy_RECLAIM_POLICY_DELETE},
		{name: "retain", sc: &storagev1.StorageClass{ReclaimPolicy: &retainPolicy}, want: ateapipb.ReclaimPolicy_RECLAIM_POLICY_RETAIN},
		// Recycle has no Substrate equivalent; keeping the disk is the safe read.
		{name: "recycle", sc: &storagev1.StorageClass{ReclaimPolicy: &recyclePolicy}, want: ateapipb.ReclaimPolicy_RECLAIM_POLICY_RETAIN},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := defaultReclaimPolicy(tc.sc); got != tc.want {
				t.Errorf("defaultReclaimPolicy() = %v, want %v", got, tc.want)
			}
		})
	}
}

// TestValidateCustomCreateExternalVolumeRequest covers the rules that relate
// the request's fields to each other, which no single-field tag can express.
