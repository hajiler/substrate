...
func TestDemoVolume(t *testing.T) {
	...
	// We deployed them via hack/install-ate.sh --deploy-demo-counter-volume
	existingWp, err := clients.SubstrateK8s.ApiV1alpha1().WorkerPools("ate-demo-counter-volume").Get(ctx, "counter-volume", metav1.GetOptions{})
	if err != nil {
		t.Fatalf("failed to get existing WorkerPool: %v", err)
	}

	existingAt, err := clients.SubstrateK8s.ApiV1alpha1().ActorTemplates("ate-demo-counter-volume").Get(ctx, "counter-volume", metav1.GetOptions{})
	if err != nil {
	...
	at := &v1alpha1.ActorTemplate{
		...
		Spec: v1alpha1.ActorTemplateSpec{
			...
			SnapshotsConfig: v1alpha1.SnapshotsConfig{
				Location: "gs://" + env["BUCKET_NAME"] + "/ate-demo-counter-volume",
			},
		},
	}
	...
	actorID := "demo-actor-vol-1-" + nsObj.Name
	...
	// Check that external volume is correctly created and populated in the Actor state
	if len(actor.GetVolumes()) != 1 {
		t.Fatalf("expected 1 volume on actor, got %d", len(actor.GetVolumes()))
	}
	vol := actor.GetVolumes()[0]
	wantVolName := actorID + "-data-vol"
	if vol.GetActorVolumeId() != wantVolName {
		t.Errorf("expected volume name %q, got %q", wantVolName, vol.GetActorVolumeId())
	}
	if vol.GetStorageVolumeId() == "" {
		t.Errorf("expected non-empty storage volume ID")
	}
	if vol.GetVolumeType() != "standard" {
		t.Errorf("expected volume type 'standard', got %q", vol.GetVolumeType())
	}
	if vol.GetStatus() != ateapipb.ExternalVolume_CREATED {
		t.Errorf("expected volume status to be CREATED, got %v", vol.GetStatus())
	}

	t.Logf("Successfully verified actor volumes list: %v", actor.GetVolumes())
}
